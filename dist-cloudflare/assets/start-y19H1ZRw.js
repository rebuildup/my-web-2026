import { a8 as createMiddleware } from './worker-entry-BHTWc-M0.js';
...
var getPath = (request) => {
	...
	for (; i < url.length; i++) {
		...
		if (charCode === 37) {
			...
			const end =
				queryIndex === -1 ? (hashIndex === -1 ? void 0 : hashIndex) : hashIndex === -1 ? queryIndex : Math.min(queryIndex, hashIndex);
...
var Hono$1 = class _Hono {
  ...
  */
	...
	#dispatch(request, executionCtx, env, method) {
		...
		if (matchResult[0].length === 1) {
			...
			return res instanceof Promise
				? res.then((resolved) => resolved || (c.finalized ? c.res : this.#notFoundHandler(c))).catch((err) => this.#handleError(err, c))
	...
	request = (input, requestInit, Env, executionCtx) => {
		...
		return this.fetch(
			new Request(/^https?:\/\//.test(input) ? input : `http://localhost${mergePath('/', input)}`, requestInit),
			Env,
			executionCtx,
		);
	};
	...
	buildRegExpStr() {
		...
		const strList = childKeys
			.map((k) => {
				...
				return childStr === ''
					? ''
					: (typeof c.#varIndex === 'number' ? `(${k})@${c.#varIndex}` : regExpMetaChars.has(k) ? `\\${k}` : k) + childStr;
			})
			.filter(Boolean);
...
function buildWildcardRegExp(path) {
	return (wildcardRegExpCache[path] ??= new RegExp(
		`^${path.replace(/\/:[^/{}]+(?:\{\[\^\/]\+})?(?=[/{]|$)|\/?\*$|([.\\+*[^\]$()?{}|])/g, (match2, metaChar) =>
			metaChar
				? `\\${metaChar}`
				: match2 === '/*'
					? TAIL_WILDCARD_REG_EXP_STR
...
const externalBoundary = new Hono();
...
const EXTERNAL_BOUNDARY_PREFIXES = ['/api/v1', '/webhooks', '/oauth', '/integrations'];
const externalBoundaryMiddleware = createMiddleware().server(async ({ next, request }) => {
	...
	if (EXTERNAL_BOUNDARY_PREFIXES.some((prefix) => url.pathname.startsWith(prefix))) {
		const honoApp = new Hono().route('/', externalBoundary);
		const ctx = {
			waitUntil: () => {},
			passThroughOnException: () => {},
		};
		return honoApp.fetch(request, {}, ctx);
	}
	return next();
});
