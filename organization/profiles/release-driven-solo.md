...
my-web-2026 の current release-driven development で使用する Operating Model を定義します。
...
- 既存の Cloudflare Workers / TanStack Start / Hono / Bun に関する ADR・root contract を project-specific authority として保持する。
- project-local `skills/` は progressive disclosure の実行面であり、同名 Skill の存在だけを freshness evidence にしない。
- deployment / secret / release surface の変更は既存 ADR と環境境界を維持して reconcile する。

## Upstream source

`rebuildup/project-init@release-0-3-0` (commit `57fb4a2e5abe6f52e4fd9cb2cb88234496e47d4b`) を基準に reconcile する。upstream の tool/provider Practice より project-local explicit decision が具体的な場合は、Constitution を満たす範囲で後者を保持する。
